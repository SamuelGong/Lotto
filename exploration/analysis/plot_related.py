...
def stacked_bar_plot(x_label, y_label, data, figure_file_name,
    ...
    if customized_params:
        if 'yaxis_time' in customized_params \
            and customized_params['yaxis_time']:
            network_division_factor = 1.0
            if 60 <= max_bar_height < 7200:
                network_division_factor = 60
                y_label += ' (min)'
            elif max_bar_height >= 7200:
                network_division_factor = 3600
                y_label += ' (h)'
            else:
                y_label += ' (s)'
            ...
            for group, group_dict in data.items():
                for member, member_dict in group_dict.items():
                    for element, value in member_dict.items():
                        member_dict[element] = value / network_division_factor
                    bar_height_dict[group][member] /= network_division_factor
            max_bar_height /= network_division_factor
        elif 'yaxis_network' in customized_params \
                and customized_params["yaxis_network"]:
            network_division_factor = 1.0
            if 1024 <= max_bar_height:
                network_division_factor = 1024
                y_label += ' (GB)'
            # elif max_bar_height >= 7200:
...
def preprocess_accuracy(data, log=False, y_to_min_max=False,
    ...
    for _idx, _data in enumerate(data):
        y = _data['y']
        if reverse_when_preprocess_acc:
            y = -y
        if 'label' in _data:
            label = _data['label']
        else:
    ...
    if log or y_to_min_max:
        for _idx, _data in enumerate(data):
            ...
            if len(_i) > 0:
                ...
                x_ind_to_min_max_y = _i[-1]
            else:
                x_ind_to_min_max_y = 0
            x_to_min_max_y = x[x_ind_to_min_max_y]

            if log:
                last_avg_period = 10  # TODO: avoid hard-coding
                print(f'\t{label}: max_y: {round(max(y), 2)}, '
                      f'last_y: {round(y[-1], 2)}, '
                      f'last_{last_avg_period}_avg: '
                      f'{round(np.mean(_data["y"][-last_avg_period:]), 2)}, '
                      f'x_to_min_max_y: {x_to_min_max_y}')
...
def complex_plot(data, x_label, y_label, figure_file_name,
    ...
    if params and "linewidth" in params:
    ...
    for _idx, _data in enumerate(data):
        ...
        if 'scatter' in _data and _data['scatter']:
            ...
            ax.plot(x, y, p, label=label, color=color, linewidth=linewidth)
    ...
    if params and "xscale" in params:
    ...
    ax.yaxis.get_offset_text().set_fontsize(_fontsize)
...
def multi_lat_cdf(label_dict, xlabel, ylabel, filename,
    ...
    if display_legend:
        ...
        if params and 'legend' in params:
            legend_params.update(params['legend'])
