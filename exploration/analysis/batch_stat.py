...
coordinator_folder_rel = "lotto-coordinator"
log_rel = "log.txt"
...
def extract_app_metric(task_folder, metric):
    ...
    if "simulation" in config and "time" in config["simulation"]:
        ...
        if "inflation_factor" in config["simulation"]["time"]:
            inflation_factor = config["simulation"]["time"]["inflation_factor"]
        ...
        for line in lines:
            if "Stats update" in line:
                ...
                total_time += lat_max * inflation_factor
