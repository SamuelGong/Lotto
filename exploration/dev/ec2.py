...
def parse_cluster_config(cluster_config_path, scale=False):
    ...
    if not scale and 'server' in cluster_config:  # the server is not changed during scaling
...
def ec2_actions_on_a_cluster(action, last_response_path, launch_result_path,
    ...
    # to avoid error RequestLimitExceeded
    ...
    batch_size = 200  # TODO: avoid hard-coding and should match your own quotas approved by AWS
    for region, instance_ids in region_to_instance_ids_mapping.items():
        ...
        response = []
        cnt = 0
        for b in range(0, len(instance_ids), batch_size):
            print(f"Acting '{action}' for the {cnt}th batch of nodes.")
            e = b + batch_size
            _instance_ids = instance_ids[b:e]

            if action == 'start':
                _response = client.start_instances(InstanceIds=_instance_ids)
            elif action == 'reboot':
                _response = client.reboot_instances(InstanceIds=_instance_ids)
            elif action == 'stop':
                _response = client.stop_instances(InstanceIds=_instance_ids)
            elif action == 'terminate':
                _response = client.terminate_instances(InstanceIds=_instance_ids)
            else:
                _response = None
...
def main(args):
    ...
    if command in ["launch", "scale"]:
        ...
        last_response = engine.run(execution_plan_list, 8)
        with open(last_response_path, 'w') as fout:
            json.dump(last_response, fout, indent=4)
