...
from infra.apps.federated_learning.models import registry as models_registry
from infra.apps.federated_learning.trainers import base
from infra.apps.federated_learning.utils import optimizers
            ...
            try:
                ...
                if True:
                    ...
                    if hasattr(config, 'lr_schedule'):
                        logging.info(f"Getting LR Schedule: {config.lr_schedule}")
                        iterations_per_epoch = np.ceil(len(trainset) /
                                                       batch_size).astype(int)
                        # acutally the latter two is not necessarily used for all LRs
            ...
            try:
                ...
                if callable(custom_test):
                    ...
                    test_loader = torch.utils.data.DataLoader(
                        testset, batch_size=test_batch_size, shuffle=False, drop_last=False
                    )
                        ...
                        for batch_id, (examples, labels) in enumerate(test_loader):
                            ...
                            if batch_id % log_interval == 0:
                                if 'albert' in model_name:  # actually it is perplexity
                                    _overall_loss = overall_loss / (batch_id + 1)
                                    accuracy = np.exp(_overall_loss)
                                else:
                                    accuracy = correct / total
                                logging.info(
                                    f"[Client #{self.client_id} #{os.getpid()}] "
                                    f"Epoch: [{batch_id}/{len(test_loader)}]\t"
                                    f"Current Acc: {round(accuracy, 6)}")
