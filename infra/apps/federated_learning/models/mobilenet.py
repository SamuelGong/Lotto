# Reference: https://github.com/SymbioticLab/FedScale/fedscale/utils/models/cv_models/mobilenetv2.py
...
from infra.apps.federated_learning.models.common \
    import conv1x1, conv1x1_block, conv3x3_block, dwconv3x3_block
...
class Model(nn.Module):
    ...
    def is_valid_model_type(model_type):
        return (model_type.startswith('mobilenet_')
                and len(model_type.split('_')) == 3
                and model_type.split('_')[1] in [
                    "v2", "v2b"
                ]
                and model_type.split('_')[2] in [
                    "w1", "w3d4", "wd2", "wd4",
                ])
    ...
    def get_model(model_type, num_classes=10):
        if not Model.is_valid_model_type(model_type):
            raise ValueError(
                'Invalid Mobilenet model type: {}'.format(model_type))

        in_channels = 3
        in_size = (224, 224)
        # for debug only
        # if Config().app.data.datasource == "FEMNIST":
        #     in_channels = 1
        #     in_size = (28, 28)

        resnet_type = model_type[len("mobilenet_"):]
        if resnet_type == "v2_w1":
            result = mobilenetv2_w1(num_classes=num_classes, in_channels=in_channels, in_size=in_size)
        elif resnet_type == "v2_w3d4":
            result = mobilenetv2_w3d4(num_classes=num_classes, in_channels=in_channels, in_size=in_size)
        elif resnet_type == "v2_wd2":
            result = mobilenetv2_wd2(num_classes=num_classes, in_channels=in_channels, in_size=in_size)
        elif resnet_type == "v2_wd4":
            result = mobilenetv2_wd4(num_classes=num_classes, in_channels=in_channels, in_size=in_size)
        elif resnet_type == "v2b_w1":
            result = mobilenetv2b_w1(num_classes=num_classes, in_channels=in_channels, in_size=in_size)
        elif resnet_type == "v2b_w3d4":
            result = mobilenetv2b_w3d4(num_classes=num_classes, in_channels=in_channels, in_size=in_size)
        elif resnet_type == "v2b_wd2":
            result = mobilenetv2b_wd2(num_classes=num_classes, in_channels=in_channels, in_size=in_size)
        elif resnet_type == "v2b_wd4":
            result = mobilenetv2b_wd4(num_classes=num_classes, in_channels=in_channels, in_size=in_size)
        else:
