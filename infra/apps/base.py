...
from infra.utils.misc import calc_sleep_time, mocking_cpu_intensive
from abc import abstractmethod
from infra.utils.multiprocess_handler import CustomizedMPBase
...
class App(CustomizedMPBase):
    ...
    def cleaning_loop(self):
        ...
        while True:
            ...
            if self.to_stop:
                logging.info(f"[#{my_pid}] App.cleaning_loop() "
                             f"received to_stop signal and "
                             f"issued exit_after_traverse signal.")
                ...
                for key, status in status_dict.items():
                    if status["done"] or exit_after_traverse:
                        ...
                        try:
                            ...
                            continue
                            # the program has already dead
                            # sometimes it is desired behavior
                            # think of training processes that already ended
                        logging.info(f"[#{my_pid}] App.cleaning_loop() "
            ...
            if exit_after_traverse:
                logging.info(f"[#{my_pid}] App.cleaning_loop() "
                             f"received exit_after_traverse signal.")
                break
    ...
    def running_loop(self):
        ...
        for message in sub.listen():
            ...
            if channel == CLOSE_SERVER:
                self.to_stop = True
                logging.info(f"[Server #{my_pid}] AppServer.running_loop() "
                             f"received CLOSE_SERVER signal "
                             f"and issued to_stop signal.")
                break
