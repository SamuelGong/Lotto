...
class ClientEvents(socketio.AsyncClientNamespace):
    def __init__(self, namespace, infra_client):
        super().__init__(namespace)
        self.infra_client = infra_client
        self.client_id = infra_client.client_id
    ...
    def on_disconnect(self):
        logging.info("%s The server disconnected the connection.",
                     self.log_prefix_str)
        self.infra_client.close()

    def on_connect_error(self, data):
        # logging.info("%s A connection attempt to the server failed.",
        #              self.log_prefix_str)
        pass  # to avoid too many prompts

    async def on_chunk(self, data):
        await self.infra_client.chunk_arrived(data['data'], data['ref'])

    async def on_payload(self, data):
        await self.infra_client.payload_arrived(data['id'], data['ref'])

    def on_payload_done(self, data):
        self.infra_client.payload_done(data['id'], data['ref'])
...
class ClientProcess(Process, ShareBase):
    ...
    async def _run(self):
        ...
        self.sio.register_namespace(
            ClientEvents(namespace='/', infra_client=self))
        ...
        # connection_gap = np.random.choice([2, 3, 5, 7, 11])  # TODO: avoid hard-coding
        connection_gap = 1
        log_interval = 60
        waited_cnt = 0
        while True:
            try:
                ...
                if waited_cnt % log_interval == 0:
                    logging.info(
                        "%s The server is not online (%s). "
                        "Try to reconnect every %ss.", self.log_prefix_str,
                        e, connection_gap)
