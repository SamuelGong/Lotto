...
from infra.utils.share_memory_handler \
    import ShareBase, SAMPLED_CLIENTS, \
    TRACE_RELATED, NEIGHBORS_DICT, NUM_SAMPLED_CLIENTS
...
class ClientSamplePluginServer(ShareBase):
    ...
    def client_id_transform(self, client_id, round_idx=None,
        ...
        if hasattr(Config().clients, "resource_saving") \
            ...
            if mode == "to_logical":
                ...
                # logging.info(f"[Debug] {sampled_clients}")
...
class ClientSamplePluginClient(ShareBase):
    def __init__(self, client_id):
        ShareBase.__init__(self, client_id=client_id)
    ...
    def get_num_sampled_clients(self, round_idx):
        ...
        return num_sampled_clients

    def set_sampled_clients(self, round_idx, sampled_clients):
        self.set_a_shared_value(
            key=[SAMPLED_CLIENTS, round_idx],
            value=sampled_clients
        )

    def get_sampled_clients(self, round_idx):
        sampled_clients = self.get_a_shared_value(
            key=[SAMPLED_CLIENTS, round_idx]
        )
        return sampled_clients
