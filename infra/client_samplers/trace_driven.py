...
from infra.utils.trace_related import read_trace, \
...
class ClientSampler(base.ClientSampler):
    ...
    def sample(self, candidates, round_idx, log_prefix_str,
               seed=None, save_result=True):  # TODO: this does not account for attendance
        ...
        while True:  # because we may not necessarily have enough clients to sample
            ...
            if len(actual_candidates) < sample_size:
                ...
                if seed is None:
                    ...
                    sampled_clients = rng.choice(actual_candidates,
                                                 sample_size,
                                                 replace=False)
