...
from infra.utils import multiprocess_handler
from infra.utils.quantizer import quantize
from infra.utils.batcher import batch
from infra.utils.misc import get_chunks_idx
from infra.utils.share_memory_handler import \
    TO_PUBLISH_SEND_TASK, NUM_SAMPLED_CLIENTS
# from infra.utils.trace_related import find_surviving
from infra.client_samplers.base \
    import ClientSamplePluginServer, ClientSamplePluginClient
from infra.clients.delay_mocking_plugin import DelayMockingPlugin
...
class ProtocolServer(Protocol, ClientSamplePluginServer, ABC):
    def __init__(self, client_id):
        Protocol.__init__(self, client_id=client_id)
        ClientSamplePluginServer.__init__(self, client_id=client_id)
        ...
        self.set_online_graph_dict()
        self.set_offline_graph_dict()
    ...
    def gradually_publish_a_value(self, channel, message, send_list, key_postfix):
        ...
        # if mp.cpu_count() == 80:  # TODO: remove this workaround (originall for Shaohuai's gpu1)
        #     num_cpus_at_a_time = 10
        # else:
        num_cpus_at_a_time = mp.cpu_count() // 8  # TODO: avoid hard-coding
    ...
    def get_graph_dicts(self):
        if len(self.offline_graph_dict) == 0:
            return [self.online_graph_dict]
        else:
            return [self.online_graph_dict, self.offline_graph_dict]

    def execute_a_task(self, task_info=None, type="online"):
        round_idx, chunk_idx, phase_idx = task_info
        if type == "offline":  # offline preparation
            self.spawn_to_handle(
                aux=('active', round_idx, chunk_idx, phase_idx),
                routine=self.offline_graph_dict[phase_idx]["worker"],
                args=(round_idx, chunk_idx)
            )
        else:  # online process
            self.spawn_to_handle(
                aux=('active', round_idx, chunk_idx, phase_idx),
                routine=self.online_graph_dict[phase_idx]["worker"],
                args=(round_idx, chunk_idx)
            )
    ...
    def wait_for_possible_clients(self, round_idx, chunk_idx,
        ...
        if self.if_use_next_round_threshold(phase_idx):
            num_sampled_clients \
    ...
    def threshold_test(self, round_idx, chunk_idx, phase_idx):
        ...
        cnt = len(keys)
        if cnt < self.get_threshold(round_idx=round_idx, phase_idx=phase_idx):
            return False
        else:
...
class ProtocolClient(Protocol, DelayMockingPlugin, ClientSamplePluginClient, ABC):
    def __init__(self, client_id):
        Protocol.__init__(self, client_id)
        DelayMockingPlugin.__init__(self, client_id)
        ClientSamplePluginClient.__init__(self, client_id=client_id)
        self.online_routine = {}
        self.offline_routine = {}
        self.set_online_routine()
        self.set_offline_routine()
        ...
        if isinstance(payload, dict):
            ...
            if "dropout" in payload and payload["dropout"]:  # mocking dropout
                logging.info("%s Mocking dropout at %s.",
                             log_prefix_str, self.online_routine[phase_idx])
                self.spawn_to_handle(
                ...
                if round_idx == -1:
                    logging.info("%s Received to run %s.",
                                 log_prefix_str, self.offline_routine[phase_idx])

                    self.spawn_to_handle(
                        aux=('passive', round_idx, chunk_idx, phase_idx),
                        routine=self.offline_routine[phase_idx],
                    ...
                    self.spawn_to_handle(
                        aux=('passive', round_idx, chunk_idx, phase_idx),
                        routine=self.online_routine[phase_idx],
                        args=(payload, round_idx, chunk_idx,
                              phase_idx, logical_client_id),
