...
from infra.protocols import plaintext
from networkx.generators.harary_graph import *
from infra.protocols.const import SecAggConst
from infra.utils.misc import plaintext_add, \
    plaintext_aggregate, get_chunks_idx
from infra.primitives.key_agreement \
    import registry as key_agreement_registry
from infra.primitives.secret_sharing \
    import registry as secret_sharing_registry
from infra.primitives.authenticated_encryption \
    import registry as authenticated_encryption_registry
from infra.primitives.pseudorandom_generator \
    import registry as pseudorandom_generator_registry
from infra.primitives.utils \
    import rand_bytes, secagg_concatenate, secagg_separate
from infra.primitives.differential_privacy\
...
class ProtocolServer(plaintext.ProtocolServer, SecAggConst):
    ...
    def store_client_payload(self, args):
        ...
        if round_idx < 0:  # offline process
            assert chunk_idx == 0
            # for Lotto
            if phase_idx == 0:
